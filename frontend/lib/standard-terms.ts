 ...
 * templates/Mutual-NDA.md at the repository root. Free to use under CC BY 4.0.
 *
 * Body text supports two inline markers:
 *   **text**    bold (defined terms)
 *   {{Name}}    a reference to the Cover Page section with that name
 */
...
export const STANDARD_TERMS_VERSION = "1.0";
export const STANDARD_TERMS_URL =
  "https://commonpaper.com/standards/mutual-nda/1.0/";

export const STANDARD_TERMS: TermSection[] = [
  {
    title: "Introduction",
...
/** Anchor id for a Cover Page section, shared by the document and term refs. */
export function coverAnchor(name: string): string {
  return `cover-${name.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
}
